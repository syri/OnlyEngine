...
namespace Engine
{
	...
	void CD3D12Renderer::LoadPipeline()
	{
		...
		{
			CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed to create DXGI Factory."));
		}
		...
		if (m_bUseWarpDevice)
		{
			...
			{
				CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed to enumerate the Warp Adapter."));
			}

			if (FAILED(D3D12CreateDevice(WarpAdapter.Get(), D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(&m_Device))))
			{
				CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed to create the D3D12 Device."));
			}
		}
		else
		{
		...
		{
			CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed to create Command Queue."));
		}
		...
		{
			CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed to create Swap Chain."));
		}

		// TODO: Fullscreen implementation
		if (FAILED(Factory->MakeWindowAssociation(WindowDescriptor.Handle, DXGI_MWA_NO_ALT_ENTER)))
		{
			CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed to create Window Association."));
		}

		if (FAILED(SwapChain.As(&m_SwapChain)))
		{
			CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed to assign Swap Chain."));
		}
		...
		{
			CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed to create Render Target View Descriptor Heap."));
		}
		...
		for (uint32_t BufferIndex = 0; BufferIndex < s_FrameCount; BufferIndex++)
		{
			...
			{
				CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed to get Swap Chain Buffer"));
			}
		...
		{
			CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed to create Command Allocator."));
		}
	}
	...
	void CD3D12Renderer::LoadAssets()
	{
		...
		{
			CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed to create Command List."));
		}

		if (FAILED(m_CommandList->Close()))
		{
			CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed to close Command List"));
		}

		if (FAILED(m_Device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&m_Fence))))
		{
			CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed to create Fence."));
		}
		m_FenceValue = 1;
		...
		if (m_FenceEvent == nullptr)
		{
			...
			{
				CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Error was thrown by the D3D12Renderer Fence."));
			}
		}
	}
	...
	void CD3D12Renderer::Render()
	{
		...
		{
			CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed to present frame."));
		}
	...
	void CD3D12Renderer::PopulateCommandList()
	{
		...
		{
			CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed to reset Command Allocator."));
		}

		if (FAILED(m_CommandList->Reset(m_CommandAllocator.Get(), m_PipelineState.Get())))
		{
			CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed to reset Command List."));
		}
	...
	void CD3D12Renderer::WaitForPreviousFrame()
	{
		...
		{
			CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed to signal to D3D12Renderer Fence."));
		}
		m_FenceValue++;
		...
		if (m_Fence->GetCompletedValue(), Fence)
		{
			...
			{
				CLogger::PrintLine(LOG_Rendering, ELogType::Error, TEXT("Failed while waiting on Fence."));
			}
