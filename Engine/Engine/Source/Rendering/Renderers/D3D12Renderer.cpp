...
namespace Engine
{
	...
	void D3D12Renderer::LoadPipeline()
	{
		...
		{
			Logger::Print(LogType::Error, true, TEXT("Failed to create DXGI Factory."));
		}
		...
		if (m_UseWarpDevice)
		{
			...
			{
				Logger::Print(LogType::Error, true, TEXT("Failed to enumerate the Warp Adapter."));
			}

			if (FAILED(D3D12CreateDevice(WarpAdapter.Get(), D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(&m_Device))))
			{
				Logger::Print(LogType::Error, true, TEXT("Failed to create the D3D12 Device."));
			}
		}
		else
		{
		...
		{
			Logger::Print(LogType::Error, true, TEXT("Failed to create Command Queue."));
		}
		...
		{
			Logger::Print(LogType::Error, true, TEXT("Failed to create Swap Chain."));
		}
		...
		{
			Logger::Print(LogType::Error, true, TEXT("Failed to create Window Association."));
		}

		if (FAILED(SwapChain.As(&m_SwapChain)))
		{
			Logger::Print(LogType::Error, true, TEXT("Failed to assign Swap Chain."));
		}
		...
		{
			Logger::Print(LogType::Error, true, TEXT("Failed to create Render Target View Descriptor Heap."));
		}
		...
		for (unsigned int BufferIndex = 0; BufferIndex < m_FrameCount; BufferIndex++)
		{
			...
			{
				Logger::Print(LogType::Error, true, TEXT("Failed to get Swap Chain Buffer"));
			}
		...
		{
			Logger::Print(LogType::Error, true, TEXT("Failed to create Command Allocator."));
		}
	}
	...
	void D3D12Renderer::LoadAssets()
	{
		...
		{
			Logger::Print(LogType::Error, true, TEXT("Failed to create Command List."));
		}

		if (FAILED(m_CommandList->Close()))
		{
			Logger::Print(LogType::Error, true, TEXT("Failed to close Command List"));
		}

		if (FAILED(m_Device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&m_Fence))))
		{
			Logger::Print(LogType::Error, true, TEXT("Failed to create Fence."));
		}
		m_FenceValue = 1;
		...
		if (m_FenceEvent == nullptr)
		{
			...
			{
				Logger::Print(LogType::Error, true, TEXT("Error was thrown by the D3D12Renderer Fence."));
			}
		}
	}
	...
	void D3D12Renderer::Render()
	{
		...
		{
			Logger::Print(LogType::Error, true, TEXT("Failed to present frame."));
		}
	...
	void D3D12Renderer::PopulateCommandList()
	{
		...
		{
			Logger::Print(LogType::Error, true, TEXT("Failed to reset Command Allocator."));
		}

		if (FAILED(m_CommandList->Reset(m_CommandAllocator.Get(), m_PipelineState.Get())))
		{
			Logger::Print(LogType::Error, true, TEXT("Failed to reset Command List."));
		}
	...
	void D3D12Renderer::WaitForPreviousFrame()
	{
		...
		{
			Logger::Print(LogType::Error, true, TEXT("Failed to signal to D3D12Renderer Fence."));
		}
		m_FenceValue++;
		...
		if (m_Fence->GetCompletedValue(), Fence)
		{
			...
			{
				Logger::Print(LogType::Error, true, TEXT("Failed while waiting on Fence."));
			}
