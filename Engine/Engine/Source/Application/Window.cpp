...
namespace Engine
{
	Window::Window(WindowDescriptor* InWindowDescriptor, const RendererType InRendererType)
		: m_WindowDescriptor(InWindowDescriptor), m_RendererType(InRendererType)
	{
		...
		WindowClass.lpszClassName = m_WindowDescriptor->WindowTitle;
		...
		if (m_WindowDescriptor->Fullscreen)
		{
			...
			m_WindowDescriptor->WindowHandle = CreateWindowEx(WS_EX_APPWINDOW, m_WindowDescriptor->WindowTitle, m_WindowDescriptor->WindowTitle,
	...
	Window::~Window()
	{
		...
		UnregisterClass(m_WindowDescriptor->WindowTitle, m_InstanceHandle);
